...
{
  if (self.theCat) {
    self.catImageView.image     = [UIImage imageWithData:[NSData dataWithContentsOfFile:self.theCat.imagePath]];
...
- (IBAction)save:(id)sender {
  if (!self.catTitleTextField.text.length ) {
    ...
    if (self.theCat) {
      if (!self.imagePath) {
        self.imagePath = self.theCat.imagePath;
      }
      [self.theCat updateWithImagePath:self.imagePath andTitle:self.catTitleTextField.text];
    } else {
      self.theCat = [[CARCat alloc] init];
      self.theCat.imagePath  = self.imagePath;
